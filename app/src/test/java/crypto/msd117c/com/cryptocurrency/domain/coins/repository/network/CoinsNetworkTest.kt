...
class CoinsNetworkTest : BaseTest(2) {
    ...
    fun init() {
        ...
        `when`(service.retrieveCoins()).thenReturn(Observable.just(coinResponse))
    }

    @Test
    fun test01ResponseOk() {
        testObserver = coinsNetwork.retrieveLatestCoins().test()
    ...
    fun test02ResponseKo() {
        `when`(service.retrieveCoins()).thenReturn(Observable.error(IOException()))
