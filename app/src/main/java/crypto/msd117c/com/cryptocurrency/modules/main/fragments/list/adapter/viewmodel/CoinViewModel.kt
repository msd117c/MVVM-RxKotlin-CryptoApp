package crypto.msd117c.com.cryptocurrency.modules.main.fragments.list.adapter.viewmodel

import androidx.lifecycle.ViewModel
import crypto.msd117c.com.cryptocurrency.data.model.list.Coin
import crypto.msd117c.com.cryptocurrency.util.comfy
...
class CoinViewModel(private val coin: Coin) : ViewModel() {
    ...
    fun getPercentChange1h(): String =
        "${coin.quote.usd.percentChange1h?.toString().normalize}% / 1 hour"

    fun getPercentChange24h(): String =
        "${coin.quote.usd.percentChange24h?.toString().normalize}% / 24 hours"

    fun getPercentChange7d(): String =
        "${coin.quote.usd.percentChange7d?.toString().normalize}% / 7 days"
